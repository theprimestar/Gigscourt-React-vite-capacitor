...
export default function ChatScreen({
  ...
  const [messages, setMessages] = useState(() => {
    const cacheKey = MSG_CACHE_PREFIX + (chatId || otherUserId);
  ...
  const [otherUser, setOtherUser] = useState(() => {
    ...
    return getCached(PROFILE_CACHE_PREFIX + otherUserId) || null;
  });
  ...
  // ── Dismiss actions on outside tap ─
  useEffect(() => {
    if (!actionMsg) return;
    const close = e => {
      if (!actionBarRef.current?.contains(e.target)) {
        setActionMsg(null);
      }
    };
  ...
  const init = async () => {
    ...
    try {
      ...
      if (isMounted.current && channelData) {
        if (
          channelData.banner_dismissed_at &&
          channelData.banner_dismissed_by === user.id
        ) {
      ...
      if (isMounted.current && profile) {
        ...
        if (
          !cachedProfile ||
          cachedProfile.full_name !== profile.full_name ||
          cachedProfile.profile_pic_url !== profile.profile_pic_url
        ) {
  ...
  const subscribeToChannel = channelId => {
    ...
    channelRef.current.on('broadcast', { event: 'message' }, payload => {
      ...
      setMessages(prev => {
        const updated = [...prev, { ...msg, status: 'sent' }];
  ...
  const startPolling = (channelId, userId) => {
    ...
    pollIntervalRef.current = setInterval(async () => {
      ...
      // Gig status
      const currentGig = await getGigForChannel(channelId);
      if (isMounted.current) {
        setGig(prev => {
          if (JSON.stringify(prev) !== JSON.stringify(currentGig)) {
            return currentGig;
          }
          return prev;
        });
      ...
      setMessages(prev => {
        ...
        data.forEach(m => {
          if (!seenIds.current.has(m.id)) {
            ...
            if (existing && m.is_read && existing.status !== 'read') {
              merged.set(m.id, {
                ...existing,
                is_read: true,
                status: 'read',
      ...
      if (currentGig && isMounted.current) {
        checkGigReminder(currentGig);
      }
    }, 5000);
  };
  ...
  const handleLongPress = async (e, msg) => {
    e.preventDefault();
    await triggerHaptic();
    setActionMsg(actionMsg?.id === msg.id ? null : msg);
    setReplyingTo(null);
    setEditingMsg(null);
  };
  ...
  const handleCopy = text => {
    navigator.clipboard.writeText(text).then(() => setActionMsg(null)).catch(() => {});
  };
    ...
    if (isMine) {
      ...
      setMessages(prev => {
        const updated = prev.map(m =>
          m.id === msgId ? { ...m, deleted_by_sender: true } : m
        );
        setCached(msgCacheKey, updated);
        return updated;
      });
  ...
  const submitEdit = async () => {
    ...
    setMessages(prev => {
      const updated = prev.map(m =>
        m.id === editingMsg.id
          ? { ...m, text: editText.trim(), edited_at: new Date().toISOString() }
          : m
      );
      setCached(msgCacheKey, updated);
      return updated;
  ...
  const canEdit = msg => {
    return (
      ...
      !msg.image_url &&
      !msg.audio_url &&
      Date.now() - new Date(msg.created_at).getTime() < 600000
    );
  };
  ...
  const handleRegisterGig = async () => {
    ...
    try {
      const result = await registerGig(
  ...
  const checkGigReminder = async currentGig => {
    ...
    if (
      currentUserId === currentGig.client_id &&
      shouldSendReminder(currentGig)
    ) {
      if (otherUser?.onesignal_player_id) {
        fetch(PUSH_NOTIFICATION_URL, {
          ...
          body: JSON.stringify({
            ...
            contents: {
              en: `Please rate your experience with ${otherUserName || 'the provider'}`,
            },
  ...
  const sendTextMessage = async (text, tempId, replyToId = null) => {
    ...
    try {
      ...
      if (replyToId) {
        ...
        savedMessage.reply_to_id = replyToId;
      }
  ...
  const uploadAudio = async blob => {
    ...
    try {
      ...
      formData.append('fileName', 'voice-message.webm');
    ...
    <div className="chat-screen">
      ...
      <div className="chat-header">
        ...
        <button
          ...
          className="chat-header-gig-btn"
          disabled={gigCheckRunning}
        >
          {getGigButtonLabel()}
        </button>
      </div>
      ...
      {shouldShowBanner() && getBannerText() && (
        <div className="gig-banner">
          ...
          {(!gig || gig.status !== 'pending_review') && (
            <button onClick={handleDismissBanner} className="gig-banner-dismiss">
              ✕
            </button>
          )}
        </div>
      ...
      <div className="chat-messages" ref={chatContainerRef}>
        ...
        {messages.map(msg => {
          ...
          return (
            <div
              key={msg.id}
              className={`message-row ${isMine ? 'message-mine' : 'message-other'} ${
                actionMsg && !isActionTarget ? 'message-dimmed' : ''
              }`}
            >
              <div
                className={`message-bubble ${isMine ? 'bubble-mine' : 'bubble-other'} ${
                  isDeleted ? 'bubble-deleted' : ''
                } ${isActionTarget ? 'bubble-highlighted' : ''}`}
              ...
              >
                {isDeleted ? (
                  <p className="message-text">This message was deleted</p>
                ) : (
                  <>
                    {repliedMsg && (
                    ...
                    {editingMsg?.id === msg.id ? (
                      <div className="edit-inline">
                        <input
                          ...
                          autoFocus
                        />
                        <button onClick={submitEdit} className="edit-inline-save">
                          Save
                        </button>
                        <button
                          onClick={() => setEditingMsg(null)}
                          className="edit-inline-cancel"
                        >
                          ✕
                        </button>
                      </div>
              ...
              {/* Long Press Actions — floating overlay below the bubble, right-aligned */}
              {isActionTarget && (
                <div
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => handleDeleteMsg(msg.id, isMine)}
                    aria-label="Delete"
                  >
                    <IconDelete />
                  </button>
        ...
        <div className="reply-preview">
          <div className="reply-preview-text">
            ...
            <div className="reply-preview-msg">
              {replyingTo.text ||
                (replyingTo.image_url ? 'Photo' : 'Voice message')}
            </div>
          </div>
          <button
      ...
      {isRecording ? (
        <div className="voice-recording-bar">
          ...
          <span className="recording-timer">
            0:{recordingTime < 10 ? '0' : ''}
            {recordingTime}
          </span>
          <button onClick={cancelRecording} className="recording-cancel-btn">
            Cancel
          </button>
        </div>
      ) : (
      ...
      {showReviewForm && (
        ...
        >
          <div
            className="bottom-sheet review-sheet"
          ...
          >
            ...
            <div className="bottom-sheet-content">
              ...
              <p style={{ color: '#8e8e93', fontSize: 13, marginBottom: 16 }}>
                How was your gig with{' '}
                {otherUser?.full_name || otherUserName || 'the provider'}?
              </p>
      ...
      {/* Dismiss overlay for long press actions */}
      {actionMsg && (
        <div
