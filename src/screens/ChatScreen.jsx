...
const IconBack = () => (<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>);
const IconAvatar = () => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4.4 3.6-8 8-8s8 3.6 8 8"/></svg>);
const IconReply = () => (<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 17 4 12 9 7"/><path d="M20 18v-2a4 4 0 0 0-4-4H4"/></svg>);
const IconCopy = () => (<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>);
const IconDelete = () => (<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>);
const IconEdit = () => (<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>);
const IconAddPhoto = () => (<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>);
const IconMic = () => (<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/></svg>);
const IconSend = () => (<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>);
...
function ChatScreen({ chatId, otherUserId, otherUserName, onBack, onViewProfile, isVisible }) {
  ...
  const init = async () => {
    setError(null); seenIds.current = new Set();
    try {
  ...
  return (
    <div className="chat-screen">
      ...
      {shouldShowBanner() && getBannerText() && (
        <div className="gig-banner"><div className="gig-banner-scroll"><p className="gig-banner-text">{getBannerText()}</p><p className="gig-banner-text">{getBannerText()}</p></div>{(!gig || gig.status !== 'pending_review') && <button onClick={handleDismissBanner} className="gig-banner-dismiss">✕</button>}</div>
      )}
      {showReviewForm && (<div className="bottom-sheet-overlay" onClick={() => setShowReviewForm(false)}><div className="bottom-sheet review-sheet" onClick={e => e.stopPropagation()}><div className="bottom-sheet-handle"></div><div className="bottom-sheet-content"><h2>Rate your experience</h2><p style={{ color: '#8e8e93', fontSize: 13, marginBottom: 16 }}>How was your gig with {otherUser?.full_name || otherUserName || 'the provider'}?</p><div className="star-selector">{[1,2,3,4,5].map(star => <button key={star} className={`star-btn ${star <= reviewRating ? 'star-active' : ''}`} onClick={() => setReviewRating(star)}>{star <= reviewRating ? '★' : '☆'}</button>)}</div><textarea value={reviewText} onChange={e => setReviewText(e.target.value)} placeholder="Write your review (optional)..." className="review-textarea" rows={3} /><button onClick={handleSubmitReview} disabled={reviewRating === 0 || submittingReview} className="onboarding-btn" style={{ marginTop: 12 }}>{submittingReview ? 'Submitting...' : 'Submit Review'}</button></div></div></div>)}
      <div className="chat-messages" ref={chatContainerRef}>
        {messages.length === 0 && !loading && <div className="chat-empty"><p>No messages yet. Say hello!</p></div>}
        {messages.map(msg => {
          const isMine = msg.sender_id === currentUserId; const isDeleted = msg.deleted_by_sender; const repliedMsg = msg.reply_to_id ? messages.find(m => m.id === msg.reply_to_id) : null;
          const isPlayingThis = playingAudio === msg.id; const progress = audioDuration > 0 ? (audioCurrentTime / audioDuration) * 100 : 0;
          return (<div key={msg.id} className={`message-row ${isMine ? 'message-mine' : 'message-other'}`}>
            ...
            {actionMsg?.id === msg.id && (<div className="message-actions-bar" ref={actionBarRef} style={{ position: 'absolute', [isMine ? 'right' : 'left']: 0, top: -44 }}><button className="message-action-btn" onClick={() => handleReply(msg)}><IconReply /></button><button className="message-action-btn" onClick={() => handleCopy(msg.text)}><IconCopy /></button><button className="message-action-btn danger" onClick={() => handleDeleteMsg(msg.id, isMine)}><IconDelete /></button>{isMine && !msg.image_url && !msg.audio_url && new Date() - new Date(msg.created_at) < 600000 && <button className="message-action-btn" onClick={() => handleEditMsg(msg)}><IconEdit /></button>}</div>)}
      ...
      {replyingTo && (<div className="reply-preview"><div className="reply-preview-text"><div className="reply-preview-name">Replying to {replyingTo.sender_id === currentUserId ? 'yourself' : (otherUser?.full_name || 'User')}</div><div className="reply-preview-msg">{replyingTo.text || (replyingTo.image_url ? 'Photo' : 'Voice message')}</div></div><button className="reply-preview-close" onClick={() => setReplyingTo(null)}>✕</button></div>)}
      {isRecording ? (<div className="voice-recording-bar"><div className="recording-dot"></div><span className="recording-timer">0:{recordingTime < 10 ? '0' : ''}{recordingTime}</span><span className="recording-hint">Release to send, slide to cancel</span><button onClick={stopRecording} style={{ background: '#ff3b30', border: 'none', color: 'white', padding: '8px 16px', borderRadius: 20, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Send</button></div>) : (<form onSubmit={handleSend} className="chat-input-bar"><button type="button" className="chat-photo-btn" onClick={() => fileInputRef.current?.click()}><IconAddPhoto /></button><input type="text" value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="Type a message..." className="chat-input" /><input type="file" accept="image/*" ref={fileInputRef} style={{ display: 'none' }} onChange={handlePhotoUpload} />{newMessage.trim() ? (<button type="submit" className="chat-send-btn"><IconSend /></button>) : (<button type="button" className={`chat-mic-btn ${isRecording ? 'recording' : ''}`} onMouseDown={startRecording} onMouseUp={stopRecording} onTouchStart={startRecording} onTouchEnd={stopRecording}><IconMic /></button>)}</form>)}
    </div>
  );
}
